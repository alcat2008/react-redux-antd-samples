  ...
  class ModalForm extends React.Component {
    ...
    render() {
      ...
      return (
        ...
        >
          <div>
            <Form layout='vertical' className={styles['jc-modal-form']} onSubmit={this._onSubmit}>
              {
                fields.map(field => (
                  <FormItem key={field.id} {...(formItemLayout || defaultFormItemLayout)} {...field.props}>
              ...
              <FormItem className={styles['jc-modal-form-footer']}>
                <Button key='cancel' size='large' onClick={_close}>取消</Button>
                <Button key='confirm' size='large' type='primary' htmlType='submit'>{okText || '确定'}</Button>
              </FormItem>
            </Form>
          </div>
        </Modal>
